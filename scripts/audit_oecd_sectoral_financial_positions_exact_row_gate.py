...
def audit_oecd_exact_row_gate() -> list[str]:
    ...
    if mode["status"]!="OBSERVED_SERIES_AVAILABLE":
        e.append("current successor mode must be observed while historical row-gate FAIL stays immutable")
    dc=model["dynamic_core"]
    if dc["reference_mode_ready_count"]!=10 or dc["reference_mode_required_count"]!=10:
        e.append("current successor readiness must be 10/10")
    if model["scientific_stage"].get("active_noncalibration_source_task") is not None:
        e.append("current successor source task must be closed")
