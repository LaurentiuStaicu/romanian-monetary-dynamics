...
def audit_f3_behavior_diagnostics(
    ...
    if mode["status"] != "OBSERVED_SERIES_AVAILABLE":
        errors.append(
            "current sectoral_financial_positions must retain successor OBSERVED_SERIES_AVAILABLE status"
        )
    if mode.get("assessment") != "model/dynamics/reference_mode_post_terminal_promotion_assessment_2026_09_21.json":
        errors.append("current sectoral_financial_positions must retain independent successor promotion evidence")
    dynamic = model_contract["dynamic_core"]
    if dynamic["reference_mode_ready_count"] != 10:
        errors.append("current reference-mode ready count must remain 10")
    ...
    if dynamic["reference_mode_closure_ready"] is not True:
        errors.append("current observational reference-mode closure must remain ready from independent successor evidence")
