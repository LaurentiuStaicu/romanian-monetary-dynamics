...
def main() -> None:
    ...
    check(
        core["integration"]["numerical_integration_contract"]
        == numerical_integration_path,
        "Core contract does not point to canonical numerical integration contract",
    )
    check(
        gate["testing_gate"]["numerical_integration_contract"]
        == numerical_integration_path,
        "SD gate does not point to canonical numerical integration contract",
    )
    check(
        numerical_integration["current_summary"][
            "stock_rate_partition_invariance"
        ] == "PASS",
        "Stock-rate partition invariance must pass",
    )
    check(
        numerical_integration["current_summary"][
            "first_order_delay_reference_convergence"
        ] == "PASS",
    ...
    check(
        numerical_integration["current_summary"]["model_activation_authorized"]
        is False,
