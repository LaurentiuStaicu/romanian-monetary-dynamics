...
def audit_trigger_aware_monitoring_horizon_2026_09_22()->list[str]:
    ...
    if ms["trigger_aware_monitoring_horizon"]!=HORIZON:
        errors.append("model contract does not point to post-F4 monitoring horizon")
    if bs["trigger_aware_monitoring_horizon"]!=HORIZON:
        errors.append("baseline does not point to post-F4 monitoring horizon")
    if ms["previous_trigger_aware_monitoring_horizon"]!=PREDECESSOR:
        errors.append("model contract predecessor monitoring horizon changed")
    if bs["previous_trigger_aware_monitoring_horizon"]!=PREDECESSOR:
        errors.append("baseline predecessor monitoring horizon changed")
