...
This terminal provenance state is a repository-integrity boundary, not a scientific promotion. It does not complete the Accounting Spine, validate a behavioural mechanism, authorize calibration/refit, open a holdout, activate a System Dynamics feedback loop or require a version bump. Provenance work reopens only on one of the assessment's declared evidence/change/failure triggers. At that terminal provenance checkpoint, continuation was governed by `model/registries/trigger_aware_monitoring_horizon_2026_09_22_post_ameco_access_protocol.json`; current continuation follows the canonical `scientific_stage.trigger_aware_monitoring_horizon` pointer in `model/registries/model_contract.json`. Repeated no-trigger provenance auditing is not treated as scientific progress. The public README intentionally remains user-oriented and does not duplicate this internal handoff detail.
...
### Pull-request live-provider executable parser repair — 2026-09-23

A post-#221 verification found that the generic pull-request live-provider regression gate had a script-reference parsing defect: its raw-string regular expression used a double-escaped dot, so ordinary references such as `scripts/foo.py` were not matched. The same review also exposed why a whole-workflow text scan would be scientifically and operationally wrong: `f3-materialization-audit.yml` still lists the historical network-capable `audit_qsa_accounting_coverage.py` in its path trigger, but the workflow's executed steps rebuild F3 exclusively from retained source-vintage bytes and do not invoke that live audit script.
...
### AMECO Autumn 2026 official-month monitoring refinement — 2026-09-24

Official European Commission timing evidence now narrows the next full forecast window without satisfying the fiscal reopen trigger. The Spring 2026 forecast press release states that the **Autumn 2026 Economic Forecast is expected to be presented in November 2026**. The Commission's AMECO page independently states that the old AMECO interface will remain accessible until November 2026 and will be decommissioned before the autumn forecast release.

The canonical successor monitoring horizon is now `model/registries/trigger_aware_monitoring_horizon_2026_09_24_post_ameco_autumn_month_confirmation.json` (**registry v0.9**). It preserves all six trigger identities and every existing scientific/evidence condition, but records November 2026 as the first legitimate release-presence window for the Autumn forecast and explicitly forbids a pre-November AMECO release check. No exact November publication day is known from the reviewed official surfaces, so none is inferred or preregistered.

This refinement does **not** satisfy the AMECO release trigger, identify the Redisstat dataset/series code, authorize UBLGBPS value inspection, open source materialisation, estimation, refit, model selection or holdout access, or change any System Dynamics feedback state. RMD remains in `EVIDENCE_TRIGGERED_BASELINE_HOLD`; the next dated trigger check remains the BNR monetary-policy event on **2026-10-08**, and public/repository version **0.3.0** remains unchanged.

### ECB QSA October 2026 release-context clarification — 2026-09-24

A review of the official ECB QSA release calendar resolved an ambiguity in the prior monitoring horizon. For Q2 2026 the ECB schedules two separate QSA publication milestones: **2 October 2026** for *Euro area households and non-financial corporations* and **28 October 2026** for the broader *Euro area economic and financial developments by institutional sector*. The QSA dataset itself covers quarterly financial and non-financial sector accounts, including institutional-sector transactions and stocks/balance sheets.

The canonical monitoring successor is now `model/registries/trigger_aware_monitoring_horizon_2026_09_24_post_qsa_release_context_clarification.json` (**registry v0.10**). It replaces the ambiguous single `ECB_QSA_next_data_release` field with both official milestones and their exact official calendar/dataset authorities. No claim is made that either press-release date necessarily changes every national series or the frozen bilateral topology.

Most importantly, **neither scheduled QSA release is a reopen trigger by itself**. The `accounting_counterpart_topology` trigger still requires changed official dataset topology or a genuinely new official source satisfying the frozen bilateral holder-by-issuer boundary. Routine vintages remain non-reopen events; exhausted historical probes remain closed. All other trigger content, the AMECO November timing boundary, current disposition and execution boundaries are unchanged.

RMD therefore remains in `EVIDENCE_TRIGGERED_BASELINE_HOLD`; the next dated trigger check remains **2026-10-08** for the BNR monetary-policy event, and public/repository version **0.3.0** remains unchanged.
