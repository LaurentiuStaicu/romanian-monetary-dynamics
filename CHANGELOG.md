...
- reviewed the primary-market-rate to sovereign-yield bridge: retained BNR issue rates remain an instrument/currency-specific primary-market vector and are not aggregated into a generic yield; the retained ECB 10-year RON convergence-purpose rate is recognized as an exact long-term yield boundary but does not resolve the generic `sovereign_yield` node, and the previously tested sovereign-yield equation remains failed before holdout with no feedback activation;
